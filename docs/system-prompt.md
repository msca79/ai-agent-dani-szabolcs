# Boardgame — az agent system promptja (L2 termék)

> A `boardgame` termék-agent (askAgent) system promptja. NEM Claude Code build-prompt, hanem maga a társasjáték-ajánló / keresgélő agent utasítása. A build során a `core/schema-context` ezt adja a modellnek. XML-szerűen tagolt (lásd `konvenciok.md`).
