# Boardgame — architektúra (fájlstruktúra + főbb döntések)
...
4. **Belső tool és külső MCP explicit határa.** A `packages/core` öt belső tool-t exportál a `games` táblán olvasva (`search_games`, `similar_games`, `recommend_bundle`, `compare_games`, `get_stock`); a BoardGameGeek MCP-kliens külön modul, ami külső, nyilvános API-t hív. Mindkettő ugyanabba a tool-use loopba regisztrálódik, de a kódban jól látszik, melyik tool DB-t olvas és melyik hálózatot hív.
